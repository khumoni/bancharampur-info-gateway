...
export const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <Link to={`/marketplace/${product.id}`} className="block h-full">
      <Card 
        className="group hover:shadow-lg transition-all duration-300 cursor-pointer relative overflow-hidden h-full flex flex-col"
          ...
          {showSellerInfo && (
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3 text-white">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" className="h-7 px-3 text-xs" onClick={(e) => e.preventDefault()}>
        ...
        <CardContent className="p-4 flex flex-col flex-grow">
          ...
          <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span>{new Date(product.createdAt).toLocaleDateString(language === 'bn' ? 'bn-BD' : 'en-US', { month: 'short', day: 'numeric' })}</span>
            </div>
          </div>
